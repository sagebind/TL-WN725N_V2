...
char	file_path_ex[PATH_LENGTH_MAX];
...
)
{
	...
	{
		rtw_merge_string(file_path_ex, PATH_LENGTH_MAX, rtw_phy_file_path, pFileName);

		if (rtw_is_file_readable(file_path_ex) == _TRUE)
		{
			rlen = rtw_retrive_from_file(file_path_ex, pHalData->para_file_buf, MAX_PARA_FILE_BUF_LEN);
			if (rlen > 0)
			{
				rtStatus = _SUCCESS;
